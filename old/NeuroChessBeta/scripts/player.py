...
class Model:
    _instances = {}
    def __new__(cls, _path: str = MODEL_PATH):
    ...
    def __init__(self, _path: str = MODEL_PATH):
    ...
    @classmethod
    def check(cls, path: str = MODEL_PATH):
        if any(hasattr(instance, '_initialized') for instance in cls._instances.values()):
            return True

        memory = psutil.virtual_memory()
        if memory.available < 1.8 * 1024**3:
            return False
    
        return bool(get_resource_path(path))
...
class NeuroPlayer():
    def __init__(self, game, color: str, _value: bool = False):
        self.game = game
        self.chess_board:  ChessBoard = self.game.chess_board
        self.model = Model()
        self.color = color
        self.s_timer = (PLAYER_DELAY + 0.15) * 75
            ...
            if event.button == 3:
                ...
                for legal_move in legal_moves:
                    if move_code == legal_move[:2]:
                        ...
                        if piece != '.':
                            self.chess_board.select(move_code)
                            self.chess_board.dragging_piece = (new_row, new_col, piece)
                        break
