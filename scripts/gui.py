...
class ModalWindow:
    ...
    def _wrap_text(self, text, font, max_width):
        # Сначала разделяем текст на строки по явным переносам '\n'
        lines_by_newline = text.split('\n')
        final_lines = []
        
        for line in lines_by_newline:
            # Обрабатываем каждую строку отдельно по ширине
            words = line.split(' ')
            current_line = []
            current_width = 0
            space_width = font.size(' ')[0]  # Ширина пробела
            
            for word in words:
                # Рендерим слово для определения его ширины
            ...
            # Добавляем последнюю собранную строку для этой части
            if current_line:
                final_lines.append(' '.join(current_line))
